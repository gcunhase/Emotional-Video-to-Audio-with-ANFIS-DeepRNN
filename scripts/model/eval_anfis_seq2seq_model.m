function [out_final, final_audio_reshaped, audio_filename, predicted_audio_filename] = eval_anfis_seq2seq_model(Xin_HSL_test, Xout_HSL_test, TLR_database, audio_database, audio_filename)
...
%   and output_audio_equivalent)
    ...
    out_fis_train_HSL = out_fis_train;
    ...
    if strcmp(emotion_dim, '2d')
        ...
        path_to_model_pos = strcat([model_seq2seq_save_path, 'net_', model_type, '_pos_', emotion_dim, 'mos_12']);
        path_to_model_neg = strcat([model_seq2seq_save_path, 'net_', model_type, '_neg_', emotion_dim, 'mos_12']);
    end
    ...
    for i=1:size(Xin_HSL_test, 1)-3
        ...
        if strcmp(emotion_dim, '2d')
            ...
            if (out_HSL_test_round == 1) % +
                out_tmp = test_seq2seq_model(X_temp_test(i:i+3), T_temp_test(i:i+3), path_to_model_pos);
            else  % - 0
                out_tmp = test_seq2seq_model(X_temp_test(i:i+3), T_temp_test(i:i+3), path_to_model_neg);
            end
        end
