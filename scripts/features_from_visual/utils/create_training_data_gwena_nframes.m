function [H_data,S_data,L_data,O_data,count] = create_training_data_gwena_nframes(data,video_size,G,H_data,S_data,L_data,O_data,NumMovie)
    %%% train data Á¤¸®
    %data = data(:,:,:,1:end);
    
    number_items = floor(size(data,4)/video_size);
    ...
    for i=1:number_items
        data_tmp = data(:,:,:,(i-1)*video_size+1:i*video_size);
    ...
    s2 = size(H_data_tmp{2}.test, 2);
    s3 = size(H_data_tmp{2}.test, 3);
    H_data{2}.test(:,:,NumMovie) = reshape(H_data_tmp{2}.test,[s1 s2*s3]);
    S_data{2}.test(:,:,NumMovie) = reshape(S_data_tmp{2}.test,[s1 s2*s3]);
    L_data{2}.test(:,:,NumMovie) = reshape(L_data_tmp{2}.test,[s1 s2*s3]);
    s1 = size(O_data_tmp{2}.test, 1);
    s2 = size(O_data_tmp{2}.test, 2);
    s3 = size(O_data_tmp{2}.test, 3);
    s4 = size(O_data_tmp{2}.test, 4);
    O_data{2}.test(:,:,:,NumMovie) = reshape(O_data_tmp{2}.test,[s1 s2 s3*s4]);
